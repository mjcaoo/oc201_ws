...
{
		float amplitude=10; //Wheel target speed limit //车轮目标速度限幅 cmj:default 3.5
	  ...
	  if (Car_Mode==Mec_Car) 
    {
		...
		else if (Car_Mode==Akm_Car) 
		{
			...
			double R, AngleR, Angle_Servo;
			
			// 直接角度模式：Vz 已经是前轮转向角度 (°)，左正右负
			// 转换为弧度: rad = degree * π / 180
			AngleR = Vz * 3.1415f / 180.0f;
			
			// Front wheel steering Angle limit (±45° = ±0.785 rad)
			//前轮转向角度限幅 (±45° = ±0.785 rad)
			AngleR = target_limit_float(AngleR, -0.785f, 0.785f);
			...
			// 舵机PWM值，舵机控制前轮转向角度
			// AngleR 为正（左转）→ Servo > 1500
			// AngleR 为负（右转）→ Servo < 1500
			...
			MOTOR_D.Target = 0; //Out of use //没有使用到
			Servo = target_limit_int(Servo, 750, 2250);	//Servo PWM value limit //舵机PWM值限幅
		}
