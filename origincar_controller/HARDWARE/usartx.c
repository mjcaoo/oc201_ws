    ...
    Receive_Data.buffer[Count]=Usart_Receive;
		...
		{   
            ...
            //所有模式标志位置0，为Usart3控制模式						
						...
						{
							// 判断 Vy 原始 buffer 值，区分直接角度模式和角速度模式
							u16 Vy_raw = (Receive_Data.buffer[5] << 8) | Receive_Data.buffer[6];
...
Input   : 8 bits high, 8 bits low
Output  : The target velocity of the robot on the X/Y/Z axis
函数功能：将上位机发过来目标角速度Vz，转换为阿克曼小车的右前轮转角
入口参数: 目标角速度Vz，单位：m/s，rad/s
返回  值：阿克曼小车的右前轮转角，单位：rad
**************************************************************************/
float Vz_to_Akm_Angle_cmj(float Vz)
{
	return Vz;
}
/**************************************************************************
